...
function [h,statsOut]=myPlotter(X,Y)
    ...
    cols(1,:)=0;
    jitScale=jitter*0.75; %To scale the patch by the width of the jitter
    ...
    for k=1:length(X)
      ...
      %      h(k).sdPtch=patchMaker(SD(k),[0.6,0.6,1]);
        ...
        if strcmp(style,'patch') || strcmp(style,'sdline')
            h(k).semPtch=patchMaker([prctile(Y(:,k),25) prctile(Y(:,k),75)],[1,0.7,0.7]);
