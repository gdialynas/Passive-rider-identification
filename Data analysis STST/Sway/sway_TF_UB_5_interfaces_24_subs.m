...
x_lim         = [0.1667 12];            % Y-axis limit for frequency range
x_ticks_shade = [0.17 0.5 1 2 4 8 12];
x_lim_shade   = [0.1667 12];            % Y-axis limit for frequency range
...
[average_phase_Y,std_phase_Y]=averagePhase(wtf_UB);

figure('name','Gain, phase and coherence 24 subjects and mean rider')
wfv(1,1)=0.1; % make first elements of frequency vector none zero to allow stdshade function to work
subplot(311)
    %hold on; grid on;
    stdshade(wtf_mag_UB',std_shadow,y_color,wfv,[]); hold on; grid on; box off
    xlim(x_lim_shade)
...
subplot(313)
    %hold on; grid on;
    stdshade(wcoh_UB',std_shadow,y_color,wfv,[]); hold on; grid on; box off
    line([wfv(1),wfv(301)],[wcoh_siglevel,wcoh_siglevel], 'Color', wcoh_siglevel_color, 'linewidth', wcoh_siglevel_linewidth, 'LineStyle', wcoh_siglevel_linestyle);
    ylim([0 1])
    xlim(x_lim_shade)
    xticks(x_ticks_shade)
