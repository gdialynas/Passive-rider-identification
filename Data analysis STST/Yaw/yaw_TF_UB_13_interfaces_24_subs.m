...
load('yaw_vel_signals_SI_t1_t2')     % input signal 
load('yaw_vel_upperbody_signals_SI_t1_t2')  % output signal 

%% Rename signals and choose trial number
u_vel_hexapod_signals = yaw_vel_signals_SI_t1;    % Input vel. signals 24 subjects trial 1 
%u_vel_hexapod_signals = yaw_vel_signals_SI_t2;   % Input vel. signals 24 subjects trial 2 

y_vel_upperbody_signals = yaw_vel_upperbody_signals_SI_t1;  % velocity signals 24 subjects trial 1 
%y_vel_upperbody_signals = yaw_vel_upperbody_signals_SI_t2; % velocity signals 24 subjects trial 2 

clearvars   yaw_vel_signals_SI_t1 yaw_vel_signals_SI_t2... 
            yaw_vel_upperbody_signals_SI_t1 yaw_vel_upperbody_signals_SI_t2
...
subplot(311)
    ...
    semilogx(wfv, wtf_mag_UB_mean_rider, 'linewidth', mean_linewidth, 'Color', z_color); hold on; grid on; box off
    xlim(x_lim)
    xticks(x_ticks)  
    ylabel({'Gain [-]'})
subplot(312)
    ...
    semilogx(wfv, wtf_phase_UB_mean_rider, 'linewidth', mean_linewidth, 'Color', z_color); hold on; grid on; box off
    xlim(x_lim)
    %ylim([-180  180]);
    xticks(x_ticks)
    ylabel({'Phase [deg]'})
subplot(313)
    ...
    semilogx(wfv, wcoh_UB_mean_rider, 'linewidth', mean_linewidth, 'Color', z_color); hold on; grid on; box off
    ylim([0 1])
    xlim(x_lim)
    xticks(x_ticks)
    ylabel({'\gamma^2 [-]'})
    xlabel('Frequency [Hz]');
suptitle('Yaw Ang.vel.upperbody/Ang.vel.hexapod')
...
subplot(311)
    %hold on; grid on;
    stdshade(wtf_mag_UB',std_shadow,'b',wfv,[]); hold on; grid on; box off
    xlim(x_lim_shade)
    xticks(x_ticks_shade)  
    ylabel({'Gain [-]'})
    set(gca, 'XScale', 'log')
subplot(312)
    stdshade(wtf_phase_UB',std_shadow,'b',wfv,[]); hold on; grid on; box off
    xlim(x_lim_shade)
    xticks(x_ticks_shade)     
    %ylim([-180  180]);
    ylabel({'Phase [deg]'})
...
subplot(313)
    %hold on; grid on;
    stdshade(wcoh_UB',std_shadow,'b',wfv,[]); hold on; grid on; box off
    ylim([0 1])
    xlim(x_lim_shade)
    xticks(x_ticks_shade)
    ylabel({'\gamma^2 [-]'})
