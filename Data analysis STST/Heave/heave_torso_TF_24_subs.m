...
u_acc_hexapod_signals = heave_acc_signals_SI_t1;    % Input acc. signals 24 subjects trial 1 
% u_acc_hexapod_signals = heave_acc_signals_SI_t2;   % Input acc. signals 24 subjects trial 2 

y_acc_UB_signals = heave_acc_UB_signals_SI_t1;  % Force signals 24 subjects trial 1 
% y_acc_UB_signals = heave_acc_UB_signals_SI_t2; % Force signals 24 subjects trial 2 
...
[average_phase_Z,std_phase_Z]=averagePhase(wtf_UB);
...
subplot(311)
    %hold on; grid on;
    stdshade(wtf_mag_UB',std_shadow,z_color,wfv,[]); hold on; grid on; box off
    xlim(x_lim_shade)
    ...
    title('(a)','FontWeight','Normal', 'Units', 'normalized', 'Position', [0.5, -0.25, 0]); % Set Title with correct Position
subplot(312)
    stdshade2(average_phase_Z,std_phase_Z,std_shadow,z_color,wfv,[]); hold on; grid on; box off
    xlim(x_lim_shade)
    xticks(x_ticks_shade)     
    %ylim([-180  180]);
...
subplot(313)
    ...
    stdshade(wcoh_UB',std_shadow,z_color,wfv,[]); 
    ylim([0 1])
    xlim(x_lim_shade)
    xticks(x_ticks_shade)
    ylim([0  1]);
