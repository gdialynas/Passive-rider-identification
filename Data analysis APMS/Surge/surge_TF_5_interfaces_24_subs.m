...
load('surge_acc_signals_SI_t1_t2')   % input signal (m/s^2)
load('surge_force_signals_SI_t1_t2') % output signal (N)
...
u_acc_signals = -surge_acc_signals_SI_t1;    % Acc. signals 24 subjects trial 1 
%u_acc_signals = -surge_acc_signals_SI_t2;   % Acc. signals 24 subjects trial 2 

y_force_signals = surge_force_signals_SI_t1;  % Force signals 24 subjects trial 1 
%y_force_signals = surge_force_signals_SI_t2; % Force signals 24 subjects trial 2 

clearvars   surge_acc_signals_SI_t1 surge_acc_signals_SI_t2... 
            surge_force_signals_SI_t1 surge_force_signals_SI_t2
...
figure()
subplot(3,6,1)
    plot(t,u_acc_signals(:,sub),'Color', x_color); hold on; box off
...
subplot(3,6,13)
    plot(t,zeros(size(t)),'Color', z_color); hold on; box off
    set(gca,'FontSize',fontsize_time_signals)
...
figure()
subplot(3,6,1)
    plot(t,u_acc_mean_rider,'Color', x_color); hold on; box off
    set(gca,'FontSize',fontsize_time_signals)
    ylabel({'X-axis','Acc. [m/s^2]'},'FontSize',fontsize)
    lgd = legend('Acc. and force in longitudinal direction');
    lgd.FontSize = fontsize;
...
y_force_SP_Z_24subs  = -y_force_signals{3}+0.18*y_force_signals{1};
...
% scales subjects with +180 phase to -180 phase
wtf_phase_FPL_Z = phase_scaling(wtf_phase_FPL_Z);
wtf_phase_FPL_Z = unwrap(wtf_phase_FPL_Z,tol);
wtf_phase_FPL_Z = my_phase_unwrap(wtf_phase_FPL_Z);
...
wtf_phase_FPR_Z = phase_scaling(wtf_phase_FPR_Z);
wtf_phase_FPR_Z = unwrap(wtf_phase_FPR_Z,tol);
wtf_phase_FPR_Z = my_phase_unwrap(wtf_phase_FPR_Z); 
...
wtf_phase_HBL_Z = phase_scaling(wtf_phase_HBL_Z);
wtf_phase_HBL_Z = unwrap(wtf_phase_HBL_Z,tol);
wtf_phase_HBL_Z = my_phase_unwrap(wtf_phase_HBL_Z); 
...
wtf_phase_HBR_Z = phase_scaling(wtf_phase_HBR_Z);
wtf_phase_HBR_Z = unwrap(wtf_phase_HBR_Z,tol);
wtf_phase_HBR_Z = my_phase_unwrap(wtf_phase_HBR_Z); 
...
[average_phase_X,std_phase_X]=averagePhase(wtf_SP_X);
[average_phase_Y,std_phase_Y]=averagePhase(wtf_SP_Y);
[average_phase_Z,std_phase_Z]=averagePhase(wtf_SP_Z);

[average_phase_X_HBRL,std_phase_X_HBRL]=averagePhase((wtf_HBR_X+wtf_HBL_X)/2);%merge left and right handlebar
[average_phase_Y_HBRL,std_phase_Y_HBRL]=averagePhase((wtf_HBR_Y+wtf_HBL_Y)/2);
[average_phase_Z_HBRL,std_phase_Z_HBRL]=averagePhase((wtf_HBR_Z+wtf_HBL_Z)/2);

[average_phase_X_FPRL,std_phase_X_FPRL]=averagePhase((wtf_FPR_X+wtf_FPL_X)/2);%merge left and right footpeg
[average_phase_Z_FPRL,std_phase_Z_FPRL]=averagePhase((wtf_FPR_Z+wtf_FPL_Z)/2);

figure('name','Phase std and mean rider')
wfv(1,1)=0.1; % make first elements of frequency vector none zero to allow stdshade function to work
subplot(3,3,1)
     stdshade2(average_phase_X,std_phase_X,std_shadow,'r',wfv,[]); hold on; grid on; box off
    xlim(x_lim)
%   ylim([-180  180]);
    xticks(x_ticks)
...
%   title('Seat')
    ...
    set(gca, 'XScale', 'log','FontSize',fontsize)
subplot(3,3,4)
    stdshade2(average_phase_Y,std_phase_Y,std_shadow,'g',wfv,[]); hold on; grid on; box off
    xlim(x_lim);
%   ylim([-180  180]);
    xticks(x_ticks);
    ...
    set(gca, 'XScale', 'log','FontSize',fontsize)
subplot(3,3,7)
       stdshade2(average_phase_Z,std_phase_Z,std_shadow,'b',wfv,[]); hold on; grid on; box off
  xlim(x_lim);
%   ylim([-180  180]);
    xticks(x_ticks)
    ...
    set(gca, 'XScale', 'log','FontSize',fontsize)
subplot(3,3,2)
     stdshade2(average_phase_X_FPRL,std_phase_X_FPRL,std_shadow,'r',wfv,[]); hold on; grid on; box off
    xlim(x_lim);
%   ylim([-180  180]);
    xticks(x_ticks)
%   title('Footpegs')
    set(gca, 'XScale', 'log','FontSize',fontsize)
subplot(3,3,8)
     stdshade2(average_phase_Z_FPRL,std_phase_Z_FPRL,std_shadow,'b',wfv,[]); hold on; grid on; box off
    xlim(x_lim);
%     ylim([-180  180]);
    xticks(x_ticks)
    xlabel('Frequency [Hz]');
...
subplot(3,3,3)
     stdshade2(average_phase_X_HBRL,std_phase_X_HBRL,std_shadow,'r',wfv,[]); hold on; grid on; box off
    xlim(x_lim);
%     ylim([-180  180]);
    xticks(x_ticks)
%     title('Handlebars')
    set(gca, 'XScale', 'log','FontSize',fontsize)
subplot(3,3,6)
    stdshade2(average_phase_Y_HBRL,std_phase_Y_HBRL,std_shadow,'g',wfv,[]); hold on; grid on; box off
    xlim(x_lim);
%   ylim([-180  180]);
    xticks(x_ticks)
    set(gca, 'XScale', 'log','FontSize',fontsize)
subplot(3,3,9)
    stdshade2(average_phase_Z_HBRL,std_phase_Z_HBRL,std_shadow,'b',wfv,[]); hold on; grid on; box off
   xlim(x_lim);
%   ylim([-180  180]);
    xticks(x_ticks)
    xlabel('Frequency [Hz]');
