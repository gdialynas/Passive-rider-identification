...
figure()
subplot(3,6,1)
    plot(u_acc_signals(:,sub),'Color', x_color); hold on; grid on; box off
    xlabel('Samples [-]');
...
subplot(3,6,18)
    ...
    suptitle('\bf Surge input and output signals in time domain');
...
subplot(3,6,18)
    ...
    ylabel('Magnitude [-]');
    title('Force HBR_Z');
    suptitle('\bf Surge input and output signals in frequency domain');
...
subplot(3,6,18)
    ...
    suptitle('\bf Surge input and output signals with zero mean in time domain');
...
subplot(3,5,15)
    ...
    ylim(y_lim)
    xlabel('Frequency [Hz]');
    suptitle('\bfSurge gain loglogs for individual subjects and mean rider')
...
subplot(3,5,15)
    ...
    xticks(x_ticks)
    xlabel('Frequency [Hz]');
    suptitle('\bfSurge phase semilogxs for individual subjects and mean rider')
...
subplot(3,5,15)
  ...
  semilogx(wfv, wcoh_HBR_Z_mean_rider, 'linewidth', mean_linewidth, 'Color', z_color); hold on; grid on; box off
    ...
    xticks(x_ticks)
    xlabel('Frequency [Hz]');
    suptitle('\bf Surge coherencesemilogxs for individual subjects and mean rider')
...
%     ylim([-180  180]);
    xticks(x_ticks)
     xlabel('Frequency [Hz]');
     set(gca, 'XScale', 'log')
 suptitle('\bf Surge gain plots mean rider')  
...
%     ylim([-180  180]);
    xticks(x_ticks)
     xlabel('Frequency [Hz]');
     set(gca, 'XScale', 'log')
 suptitle('\bf Surge phase plots mean rider')
...
wcoh_HBLR_Z=(wcoh_HBL_Z+wcoh_HBR_Z)/2; %merging left and right handlebar z axis
 stdshade(wcoh_HBLR_Z',std_shadow,'b',wfv,[]); hold on; grid on; box off
    ...
    xticks(x_ticks)
     xlabel('Frequency [Hz]');
     set(gca, 'XScale', 'log')
 suptitle('\bf Surge coherence plots mean rider')
...
%     yticklabels({'0.001', '0.01', '0.1', '1', '10'})
%     xlabel('Frequency [Hz]');
%     suptitle('\bfSurge normalised gain plots for individual subjects and mean rider')
