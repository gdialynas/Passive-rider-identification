...
load('pitch_input_signals_SI_t1_t2')   % input signal (m/s^2)
load('pitch_force_signals_SI_t1_t2') % output signal (N)
...
%u_ang_acc_signals = -pitch_vel_signals_SI_t1;   % Ang. Vel. signals 24 subjects trial 1 
%u_ang_acc_signals = -pitch_vel_signals_SI_t2;   % Ang. Vel. signals 24 subjects trial 2 

u_ang_acc_signals = -pitch_acc_signals_SI_t1;   % Ang. Acc. signals 24 subjects trial 1 
%u_ang_acc_signals = -pitch_acc_signals_SI_t2;   % Ang. Acc. signals 24 subjects trial 2

y_force_signals = pitch_force_signals_SI_t1; % Force signals 24 subjects trial 1 
%y_force_signals = pitch_force_signals_SI_t2; % Force signals 24 subjects trial 2 

clearvars   pitch_vel_signals_SI_t1 pitch_vel_signals_SI_t2... 
            pitch_acc_signals_SI_t1 pitch_acc_signals_SI_t2... 
            pitch_force_signals_SI_t1 pitch_force_signals_SI_t2
...
figure()
subplot(3,6,1)
    line([0,length(u_ang_acc_signals(:,sub))],[0,0],'Color', x_color); hold on; grid on; box off
    xlabel('Samples [-]');
    ylabel({'\bf X-axis','Acc. [rad/s^2]'})
    title(['Hexapod Vel.: \mu=', num2str(mean(u_ang_acc_signals(:,sub))),' \sigma=', num2str(std(u_ang_acc_signals(:,sub)))])
subplot(3,6,7)
    plot(u_ang_acc_signals(:,sub),'Color', y_color); hold on; grid on; box off
    xlabel('Samples [-]');
...
subplot(3,6,18)
    ...
    suptitle('\bf Pitch input and output signals in time domain');
...
y_force_SP_Z_24subs  =  -y_force_signals{3}+0.18*y_force_signals{1};%substracting crosstalk from vertical seat forces
...
y_force_HBL_X_24subs = -y_force_signals{8};
y_force_HBL_Y_24subs = y_force_signals{9};
y_force_HBL_Z_24subs = y_force_signals{10}+0.24*y_force_signals{8}; %substracting crosstalk from vertical handlebar forces;

y_force_HBR_X_24subs = -y_force_signals{11};
y_force_HBR_Y_24subs = -y_force_signals{12};
y_force_HBR_Z_24subs =  y_force_signals{13}+0.24*y_force_signals{11}; %substracting crosstalk from vertical handlebar forces;
...
subplot(3,6,1)
    ...
    title('Hexapod Acc.')
subplot(3,6,7)
    loglog(fv(1:N/2),U_acc_signals(1:N/2),'Color', y_color); hold on; grid on; box off
    xlabel('Frequency [Hz]');
...
subplot(3,6,18)
    ...
    ylabel('Magnitude [-]');
    title('Force HBR_Z');
    suptitle('\bf Pitch input and output signals in frequency domain');

%% Convert angular acceleration to linear acceleration
% position vectors between hexapod IMU pitch axis and the strain gauges at the interfaces (distance = meters) 
r_SP     = 0.665;
r_FPL    = sqrt((0.29^2)+(0.215^2)); % Pythagoras
r_FPR    = sqrt((0.29^2)+(0.215^2)); % Pythagoras
r_HBL_XZ = sqrt((0.65^2)+(0.925^2)); % Pythagoras
r_HBL_Y  = sqrt((0.65^2)+(0.955^2)); % Pythagoras
r_HBR_XZ = sqrt((0.65^2)+(0.925^2)); % Pythagoras
r_HBR_Y  = sqrt((0.65^2)+(0.955^2)); % Pythagoras
...
subplot(711)
    ...
    title('Linear pitch velocity SP')
subplot(712)
    plot(u_lin_acc_signals_FPL);
    ylabel('Acceleration (m/s^2)');
    title('Linear pitch velocity FPL')
subplot(713)
    plot(u_lin_acc_signals_FPR);
    ylabel('Acceleration (m/s^2)');
    title('Linear pitch velocity FPR')
subplot(714)
    plot(u_lin_acc_signals_HBL_XZ);
    ylabel('Acceleration (m/s^2)');
    title('Linear pitch velocity HBL_{XZ}')
subplot(715)
    plot(u_lin_acc_signals_HBL_Y);
    ylabel('Acceleration (m/s^2)');
    title('Linear pitch velocity HBL_Y')
subplot(716)
    plot(u_lin_acc_signals_HBR_XZ);
    ylabel('Acceleration (m/s^2)');
    title('Linear pitch velocity HBR_{XZ}')
subplot(717)
    plot(u_lin_acc_signals_HBR_Y);
    ylabel('Acceleration (m/s^2)');
    title('Linear pitch velocity HBR_Y')
    %suptitle('\bf Linear pitch accelerations at all interfaces')
...
subplot(3,6,18)
    ...
    suptitle('\bf Pitch input and output signals with zero mean in time domain');
...
subplot(3,5,15)
    ...
    ylim(y_lim_gain)
    xlabel('Frequency [Hz]');
    suptitle('\bf Pitch gain plots for individual subjects and mean rider')
...
subplot(3,5,15)
    ...
    xticks(x_ticks)
    xlabel('Frequency [Hz]');
    suptitle('\bf Pitch phase plots for individual subjects and mean rider')
...
subplot(3,5,15)
    ...
    xticks(x_ticks)
    xlabel('Frequency [Hz]');
    suptitle('\bf Pitch coherence plots for individual subjects and mean rider')
...
%     ylim([-180  180]);
    xticks(x_ticks)
     xlabel('Frequency [Hz]');
     set(gca, 'XScale', 'log')
 suptitle('\bf Pitch gain plots mean rider')
...
%     ylim([-180  180]);
    xticks(x_ticks)
     xlabel('Frequency [Hz]');
     set(gca, 'XScale', 'log')
 suptitle('\bf Pitch phase plots mean rider')
...
wcoh_HBLR_Z=(wcoh_HBL_Z+wcoh_HBR_Z)/2; %merging left and right handlebar z axis
 stdshade(wcoh_HBLR_Z',std_shadow,'b',wfv,[]); hold on; grid on; box off
    ...
    xticks(x_ticks)
     xlabel('Frequency [Hz]');
     set(gca, 'XScale', 'log')
 suptitle('\bf Pitch coherence plots mean rider')
...
%     xlabel('Subject');
%     
%% Plot the raw transfer function of the mean rider with the STD
% figure('name','Gain of mean rider with STD')
% subplot(3,5,1)
%     %hold on; grid on;
%     stdshade(wtf_mag_SP_X',std_shadow,'r',wfv,[]); hold on; grid on; box off
%     xlim([0.5 12])
...
% subplot(3,5,2)
%     hold on; grid on;
%     stdshade(wtf_mag_FPL_X',std_shadow,'r',wfv,[]); hold on; grid on; box off
%     xlim([0.5 12])
%     title('Foot peg left')
% subplot(3,5,12)
%     hold on; grid on;
%     stdshade(wtf_mag_FPL_Z',std_shadow,'b',wfv,[]); hold on; grid on; box off
%     xlim([0.5 12])
%     xlabel('Frequency [Hz]');
% subplot(3,5,3)
%     hold on; grid on;
%     stdshade(wtf_mag_FPR_X',std_shadow,'r',wfv,[]); hold on; grid on; box off
%     xlim([0.5 12])
%     title('Foot peg right')
% subplot(3,5,13)
%     hold on; grid on;
%     stdshade(wtf_mag_FPR_Z',std_shadow,'b',wfv,[]); hold on; grid on; box off
%     xlim([0.5 12])
%     xlabel('Frequency [Hz]');
% subplot(3,5,4)
%     hold on; grid on;
%     stdshade(wtf_mag_HBL_X',std_shadow,'r',wfv,[]); hold on; grid on; box off
%     xlim([0.5 12])
%     title('Handlebar left')
% subplot(3,5,9)
%     hold on; grid on;
%     stdshade(wtf_mag_HBL_Y',std_shadow,'g',wfv,[]); hold on; grid on; box off
%     xlim([0.5 12])
% subplot(3,5,14)
%     hold on; grid on;
%     stdshade(wtf_mag_HBL_Z',std_shadow,'b',wfv,[]); hold on; grid on; box off
%     xlim([0.5 12])
%     xlabel('Frequency [Hz]');
% subplot(3,5,5)
%     hold on; grid on;
%     stdshade(wtf_mag_HBR_X',std_shadow,'r',wfv,[]); hold on; grid on; box off
%     xlim([0.5 12])
%     title('Handlebar right')
% subplot(3,5,10)
%     hold on; grid on;
%     stdshade(wtf_mag_HBR_Y',std_shadow,'g',wfv,[]); hold on; grid on; box off
%     xlim([0.5 12])
% subplot(3,5,15)
%     hold on; grid on;
%     stdshade(wtf_mag_HBR_Z',std_shadow,'b',wfv,[]); hold on; grid on; box off
%     xlim([0.5 12])
%     xlabel('Frequency [Hz]');
...
%     yticklabels({'0.001', '0.01', '0.1', '1', '10'})
%     xlabel('Frequency [Hz]');
%     suptitle('\bf Pitch normalised gain plots for individual subjects and mean rider')
