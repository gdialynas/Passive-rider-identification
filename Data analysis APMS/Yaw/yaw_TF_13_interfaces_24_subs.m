...
load('yaw_input_signals_SI_t1_t2')   % input signal (m/s^2)
load('yaw_force_signals_SI_t1_t2') % output signal (N)
...
%u_ang_acc_signals = yaw_vel_signals_SI_t1;   % Ang. Vel. signals 24 subjects trial 1 
%u_ang_acc_signals = yaw_vel_signals_SI_t2;   % Ang. Vel. signals 24 subjects trial 2 

u_ang_acc_signals = yaw_acc_signals_SI_t1;   % Ang. Acc. signals 24 subjects trial 1 
%u_ang_acc_signals = yaw_acc_signals_SI_t2;   % Ang. Acc. signals 24 subjects trial 2

y_force_signals = yaw_force_signals_SI_t1; % Force signals 24 subjects trial 1 
%y_force_signals = yaw_force_signals_SI_t2; % Force signals 24 subjects trial 2 

clearvars   yaw_vel_signals_SI_t1 yaw_vel_signals_SI_t2... 
            yaw_acc_signals_SI_t1 yaw_acc_signals_SI_t2... 
            yaw_force_signals_SI_t1 yaw_force_signals_SI_t2
...
subplot(3,6,7)
    ...
    title('Hexapod Vel.')
subplot(3,6,13)
    plot(u_ang_acc_signals(:,sub),'Color', z_color); hold on; grid on; box off
    xlabel('Samples [-]');
...
subplot(3,6,18)
    ...
    suptitle('\bf yaw input and output signals in time domain');
...
subplot(3,6,18)
    ...
    ylabel('Magnitude [-]');
    title('Force HBR_Z');
    suptitle('\bf Yaw input and output signals in frequency domain');

%% Convert angular acceleration to linear acceleration
% position vectors between hexapod IMU yaw axis and the strain gauges at the interfaces (distance = meters) 
r_SP     = 1;
r_FPL    = 0.31;
r_FPR    = 0.31; 
r_HBL_XZ = 0.62; 
r_HBL_Y  = sqrt((0.62^2)+(0.17^2)); % Pythagoras
r_HBR_XZ = 0.62; 
r_HBR_Y  = sqrt((0.62^2)+(0.17^2)); % Pythagoras
...
subplot(711)
    ...
    title('Linear yaw velocity SP')
subplot(712)
    plot(u_lin_acc_signals_FPL);
    ylabel('Acceleration (m/s^2)');
    title('Linear yaw velocity FPL')
subplot(713)
    plot(u_lin_acc_signals_FPR);
    ylabel('Acceleration (m/s^2)');
    title('Linear yaw velocity FPR')
subplot(714)
    plot(u_lin_acc_signals_HBL_XZ);
    ylabel('Acceleration (m/s^2)');
    title('Linear yaw velocity HBL_{XZ}')
subplot(715)
    plot(u_lin_acc_signals_HBL_Y);
    ylabel('Acceleration (m/s^2)');
    title('Linear yaw velocity HBL_Y')
subplot(716)
    plot(u_lin_acc_signals_HBR_XZ);
    ylabel('Acceleration (m/s^2)');
    title('Linear yaw velocity HBR_{XZ}')
subplot(717)
    plot(u_lin_acc_signals_HBR_Y);
    ylabel('Acceleration (m/s^2)');
    title('Linear yaw velocity HBR_Y')
    %suptitle('\bf Linear yaw accelerations at all interfaces')
...
subplot(3,6,18)
    ...
    suptitle('\bf Yaw input and output signals with zero mean in time domain');
...
subplot(3,5,15)
    ...
    ylim(y_lim_gain)
    xlabel('Frequency [Hz]');
    suptitle('\bf Yaw gain plots for individual subjects and mean rider')
...
subplot(3,5,15)
    ...
    xticks(x_ticks)
    xlabel('Frequency [Hz]');
    suptitle('\bf Yaw phase plots for individual subjects and mean rider')
...
subplot(3,5,15)
    ...
    xticks(x_ticks)
    xlabel('Frequency [Hz]');
    suptitle('\bf Yaw coherence plots for individual subjects and mean rider')
...
%     ylim([-180  180]);
    xticks(x_ticks)
     xlabel('Frequency [Hz]');
     set(gca, 'XScale', 'log')
 suptitle('\bf Yaw gain plots mean rider')
...
%     ylim([-180  180]);
    xticks(x_ticks)
     xlabel('Frequency [Hz]');
     set(gca, 'XScale', 'log')
 suptitle('\bf Yaw phase plots mean rider')
...
wcoh_HBLR_Z=(wcoh_HBL_Z+wcoh_HBR_Z)/2; %merging left and right handlebar z axis
 stdshade(wcoh_HBLR_Z',std_shadow,'b',wfv,[]); hold on; grid on; box off
    ...
    xticks(x_ticks)
     xlabel('Frequency [Hz]');
     set(gca, 'XScale', 'log')
 suptitle('\bf Yaw coherence plots mean rider')
...
%     yticklabels({'0.001', '0.01', '0.1', '1', '10'})
%     xlabel('Frequency [Hz]');
%     suptitle('\bf Yaw normalised gain plots for individual subjects and mean rider')
